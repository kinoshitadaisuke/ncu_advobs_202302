...
#
# Time-stamp: <2023/04/13 01:48:55 (CST) daisuke>
#
...
print (f'List of FITS files for flatfielding:')
for filter_name in sorted (dict_target.keys () ):
    ...
    if not (path_nflat.exists () ):
        print (f'The flatfield file "{nflat}" does not exist.')
        print (f'Check the data!')
        sys.exit ()
    # printing information
    print (f'  {filter_name} band data:')
    print (f'    normalised flatfield = {nflat}')
    for file_fits in sorted (dict_target[filter_name].keys () ):
        path_fits = pathlib.Path (file_fits)
        flatfielded = path_fits.stem + "f.fits"
        print (f'      {file_fits} / {nflat}')
        print (f'          ==> {flatfielded}')
...
for filter_name in sorted (dict_target.keys () ):
    ...
    print (f'  {filter_name} band data: {n_files} files')
    total_files += n_files
# printing information
print (f'Total number of files to be processed:')
